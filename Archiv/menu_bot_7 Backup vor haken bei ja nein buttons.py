...
MENU_INPUT, ASK_BEILAGEN, SELECT_MENUES, BEILAGEN_SELECT, ASK_FINAL_LIST, ASK_SHOW_LIST, FERTIG_PERSONEN, REZEPT_INDEX, REZEPT_PERSONEN, TAUSCHE_SELECT, TAUSCHE_CONFIRM, ASK_CONFIRM, EXPORT_OPTIONS = range(13)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "🛠️ Setup – Übersicht aller Funktionen\n\n"
        "Du kannst alle Befehle jederzeit auch im Textfeld starten!\n"
        "Tippe: /menu | /meinefavoriten | /setup"
    )
...
# 2) Favoriten & Setup – einfache Einzelausgaben, kein Conversation-State
async def start_favs_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    user_id = str(q.from_user.id)
    favs = favorites.get(user_id, [])
    text = "ℹ️ Keine Favoriten vorhanden." if not favs else \
...
async def tausche_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Buttons nebeneinander in einer Zeile
    keyboard = [[
        InlineKeyboardButton("Ja",   callback_data="ask_yes"),
        InlineKeyboardButton("Nein", callback_data="ask_no"),
    ]]
    ...
    # Bleib im Beilagen-State, damit ask_beilagen_cb die Auswahl abfängt
    return ASK_BEILAGEN
