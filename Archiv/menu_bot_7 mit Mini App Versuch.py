...
async def open_miniapp_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Sendet einen Button, der die Telegram Mini-App öffnet."""
    try:
        url = MINIAPP_URL
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🍱 Mini-App öffnen", web_app=WebAppInfo(url=url))]])
        if update.message:
            await update.message.reply_text("Öffne die Mini-App:", reply_markup=kb)
        else:
            # Fallback für CallbackQuery
            await update.effective_message.reply_text("Öffne die Mini-App:", reply_markup=kb)
    except Exception as e:
        await update.effective_message.reply_text(f"⚠️ Konnte Mini-App nicht öffnen: {e}")

async def app_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await open_miniapp_button(update, context)
...
async def send_main_buttons(msg):
    kb = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("🍱 Mini-App öffnen", web_app=WebAppInfo(url=MINIAPP_URL))
        ]
    ])
...
def lade_gerichtebasis():
    ...
    # A–F:   Gericht | Aufwand | Typ | Küche | Ernährungsstil | Beilagen
    data  = [row[:9] for row in rows[1:]]
    df    = pd.DataFrame(
        data,
        columns=["Nummer", "Code", "Gericht", "Aufwand", "Typ", "Ernährungsstil", "Küche", "Beilagen", "Link"],
    )
...
async def fertig_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for g in ausgew:
        ...
        display_title_html = f"{name_html}{rest_html}"
        koch_text += f"\n{display_title_html}\n{ze_html}\n"
...
async def process_pdf_export_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice in ["koch", "beides"]:
        ...
        # Neue Formatierung: eine Zeile pro Gericht ohne führendes '- '
        # Format: "<HTML-Titel>: <HTML-Zutaten>"
        lines = [l for l in koch_text.splitlines() if l and not l.startswith("🍽")]
        for l in lines:
            if ": " not in l:
                continue
            title_html, ingredients_html = l.split(": ", 1)

            # HTML-Tags entfernen und Entities zurückwandeln
            title_plain = unescape(re.sub(r"<[^>]+>", "", title_html))
            ingredients_plain = unescape(re.sub(r"<[^>]+>", "", ingredients_html))

            pdf.set_x(pdf.l_margin)
            pdf.set_font("DejaVu", "B", 12)
            pdf.cell(0, 8, f"• {title_plain}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

            pdf.set_x(pdf.l_margin + 6)
            pdf.set_font("DejaVu", "", 12)
            pdf.multi_cell(pdf.epw - 6, 8, ingredients_plain)
            pdf.ln(2)
...
async def webapp_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Nimmt JSON aus der Mini-App entgegen und startet sofort die Menüplanung."""
    try:
        if not update.message or not update.message.web_app_data:
            return

        raw = update.message.web_app_data.data  # JSON-String
        payload = json.loads(raw)

        # Erwartete Struktur aus mini_app.html
        count  = int(payload.get("menu_count", 0))
        aufw   = payload.get("aufwand", {}) or {}
        light  = int(aufw.get("light", 0))
        medium = int(aufw.get("medium", 0))
        heavy  = int(aufw.get("heavy", 0))
        persons = int(payload.get("persons", 0))

        # minimale Validierung/Korrektur
        if light + medium + heavy != count:
            # Fülle 'medium' auf oder kappe auf count – so kommt dein Parser klar
            rest = max(0, count - (light + heavy))
            medium = rest
        # Personen für späteren Schritt vormerken (wird in fertig_input genutzt)
        if persons > 0:
            context.user_data["temp_persons"] = persons

        # Merke die Rohdaten (falls du später mehr übernehmen willst)
        context.user_data["miniapp_result"] = payload

        # Starte deinen bestehenden Plan-Flow direkt:
        plan_str = f"{count} ({light},{medium},{heavy})"  # exakt wie in deinem Flow erwartet
        await update.message.reply_text("✅ Mini-App Daten erhalten – ich plane jetzt dein Menü …")
        return await menu_input_direct(plan_str, update, context)

    except Exception as e:
        await update.effective_message.reply_text(f"❌ Konnte Mini-App-Daten nicht verarbeiten: {e}")
