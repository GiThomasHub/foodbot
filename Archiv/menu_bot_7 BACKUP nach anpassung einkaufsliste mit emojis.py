...
ART_ORDER = {"leicht": 1, "mittel": 2, "schwer": 3}
    ...
    try:
        ...
        if "fav_selection" in context.user_data:
            ...
            if show_debug_for(update):                                  #nur für ADMIN ersichtlich, as specified in def show_debug_for
                ...
                # Stil-Verteilung
                stil_counter = Counter(gewaehlte_gerichte["Stil"])
                stil_text = ", ".join(f"{v} x {k}" for k, v in stil_counter.items())

                # Art-Verteilung
                art_counter = Counter(gewaehlte_gerichte["Art"])
                art_text = ", ".join(f"{v} x {k}" for k, v in art_counter.items())

                # Einschränkung-Verteilung
                einschr_counter = Counter(gewaehlte_gerichte["Einschränkung"])
                ...
                debug_msg = (
                    ...
                    f"\n🎨 Stil-Verteilung: {stil_text}"
                    f"\n⚙️ Art-Verteilung: {art_text}"
                    f"\n🥗 Einschränkung-Verteilung: {einschr_text}"
                )
...
async def quickone_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Favoriten­gewichtung: Favoriten 3×, alle anderen 1×
    uid      = str(update.effective_user.id)
    user_favs = favorites.get(uid, [])  # favorites ist bereits geladen
    weights  = [3 if d in user_favs else 1 for d in remaining]

    # zufällige Auswahl mit Gewichten
    ...
    if query.data == "ask_yes":
        ...
        side_menus = [
             ...
             if any(c != 0 for c in parse_codes(
                 df_gerichte.loc[df_gerichte["Gericht"] == dish, "Beilage"].iloc[0]
             ))
         ]
...
async def ask_beilagen_for_menu(update_or_query, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = distribute_buttons_equally(buttons, max_per_row=3)
...
async def tausche(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for arg in args:
        ...
        if 0 <= idx < len(menues):
            ...
            # === Erweiterte Swap-Logik mit Fallback über Aufwand, Art, Stil, Einschränkung ===
            current_dish   = menues[idx]
            current_aufw   = aufw[idx]
            # Art-Label und weitere Attribute holen
            row_cur        = df_gerichte[df_gerichte["Gericht"] == current_dish].iloc[0]
            current_art    = ART_ORDER.get(row_cur["Art"], 2)
            current_stil   = row_cur["Stil"]
            current_eins   = row_cur["Einschränkung"]

            # 1) Roh-Pool: alle Gerichte außer aktuellem Slot und den anderen Slots
            other_sel = set(menues) - {current_dish}
            all_cands = set(df_gerichte["Gericht"]) - {current_dish} - other_sel

            # 2) History-Filter
            used = set(history.get(idx, []))
            pool = list(all_cands - used)
            if not pool:
                 # History zurücksetzen, wenn leer
                 history[idx] = [current_dish]
                 used = set(history[idx])
                 pool = list(all_cands - used)
            if not pool:
                 # kein Ersatz möglich → überspringen
                 continue
            ...
            for cand in pool:
                 ...
                 cand_aufw    = int(row_c["Aufwand"])
                 cand_art     = ART_ORDER.get(row_c["Art"], 2)
                 # Deltas und Mismatch-Flags
                 d_aufw       = abs(current_aufw - cand_aufw)
                 d_art        = abs(current_art - cand_art)
                 mismatch_st  = 0 if row_c["Stil"] == current_stil else 1
                 mismatch_ei  = 0 if row_c["Einschränkung"] == current_eins else 1
                 scored.append((cand, (d_aufw, d_art, mismatch_st, mismatch_ei)))

            # 4) Minimalen Score-Tupel finden
            min_score = min(score for _, score in scored)
            best      = [c for c, score in scored if score == min_score]

            # 5) Zufälliges Finale aus der Best-Liste
            neu = random.choice(best)
            menues[idx] = neu
            history.setdefault(idx, []).append(neu)
...
async def tausche_select_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "swap_done":
        ...
        for idx in sorted(sel):             # idx ist 1-basiert
            ...
            # Roh-Pool (ohne aktuellen und andere Slots)
            other_sel = set(menues) - {current_dish}
            all_cands = set(df_gerichte["Gericht"]) - {current_dish} - other_sel
            ...
            pool = list(all_cands - used)
            if not pool:
                history[slot] = [current_dish]
                used = set(history[slot])
                pool = list(all_cands - used)
            if not pool:
                continue
            ...
            neu = random.choice(best)
            menues[slot] = neu
            history.setdefault(slot, []).append(neu)
...
async def fertig_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    eink_text = f"\n🛒 Einkaufsliste für {personen} Personen:\n"
    ...
    for g in ausgew:
        # Beilagen-Namen per Session abrufen
        sel_nums      = sessions[user_id].get("beilagen", {}).get(g, [])
        beilagen_namen = df_beilagen.loc[
            df_beilagen["Nummer"].isin(sel_nums), "Beilage"
        ].tolist()

        # Zutaten für dieses Gericht aus dem skalierten 'zut'-DF holen
        part = zut[zut["Gericht"] == g]
        # Falls *keine* Beilage gewählt, nur Hauptgericht-Zeilen
        if not sel_nums:
            part = part[part["Typ"] == "Gericht"]

        # Einzelne Zutaten formattieren
        parts = []
...
async def restart_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if q.data == "restart_yes":
        await context.bot.send_message(chat_id, pad_message("Super, bis bald!👋"))
