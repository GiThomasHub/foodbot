...
MENU_INPUT, ASK_BEILAGEN, SELECT_MENUES, BEILAGEN_SELECT, ASK_FINAL_LIST, ASK_SHOW_LIST, FERTIG_PERSONEN, REZEPT_INDEX, REZEPT_PERSONEN, TAUSCHE_SELECT, TAUSCHE_CONFIRM, ASK_CONFIRM, EXPORT_OPTIONS, FAV_OVERVIEW, FAV_DELETE_SELECT = range(15)
...
def build_fav_numbers_keyboard(total: int, selected: set[int]) -> InlineKeyboardMarkup:
    """Zahlen-Buttons (max. 8 pro Zeile) + 'Fertig'."""
    btns = [
        InlineKeyboardButton(f"{'✅ ' if i in selected else ''}{i}", callback_data=f"fav_del_{i}")
        for i in range(1, total + 1)
    ]
    rows = [btns[j:j + 8] for j in range(0, len(btns), 8)]
...
# 1) Menü-Button – startet denselben Conversation-Flow wie /menu
async def menu_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return MENU_INPUT          # Einstieg in den bestehenden /menu-Conversation-State
...
async def start_setup_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(
        ...
        "/tausche – tauscht Gerichte aus\n"
        "/fertig – Einkaufs- & Kochliste erstellen\n\n"
        "/favorit x – speichert Gericht x\n"
        "/meinefavoriten – zeigt Favoriten\n"
        "/delete x – löscht Favorit x\n\n"
        "/status – aktuelle Auswahl\n"
        "/reset – Session zurücksetzen\n"
        "/setup – alle Kommandos\n"
...
async def menu_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # → Neue Zwischenfrage: passen die Gerichte?
...
async def beilage_select_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "beilage_done":
        ...
        # 3) Neue Frage
        text += "\nFür wie viele Personen soll die Einkaufsliste erstellt werden?"

        # 4) Sende ohne Inline-Keyboard
...
async def fertig_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1a. Einkaufsliste für Export speichern
    context.user_data["einkaufsliste_df"] = eink

    # 1b.  Kochliste für Export speichern
    context.user_data["kochliste_text"]     = koch_text

    # 2. Menü für alle drei Optionen anzeigen
    keyboard = InlineKeyboardMarkup([
        [ InlineKeyboardButton("🛒 In Bring! exportieren", callback_data="export_bring") ],
        [ InlineKeyboardButton("📄 Als PDF exportieren",   callback_data="export_pdf")   ],
        [ InlineKeyboardButton("🔄 Neustart",             callback_data="restart")      ],
    ])
    await update.message.reply_text(
        "Deine Listen sind bereit – was möchtest du tun?",
        reply_markup=keyboard
    )
    #return ConversationHandler.END
...
async def export_to_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Generiert ein PDF mit Einkaufs- und Kochliste (inkl. Emoji/Unicode) und sendet es."""
    ...
    # 1) Unicode-Font registrieren (lade die .ttf in dein Projektverzeichnis, z.B. /fonts)
    #    Du brauchst z.B. DejaVuSans.ttf und DejaVuSans-Bold.ttf im Ordner "./fonts/"
    # ohne Subsetting (kein fontTools.subset-Aufruf mehr)
    pdf.add_font("DejaVu",   "",  "fonts/DejaVuSans.ttf")
    pdf.add_font("DejaVu",   "B", "fonts/DejaVuSans-Bold.ttf")

    # Emoji-Font ebenfalls ohne Subsetting
    #pdf.add_font("Emoji",    "",  "fonts/NotoEmoji.ttf",        subset=False)
    ...
    # --- dynamischer Einzug für die Kategorie ---

    pdf.set_font("DejaVu", "", 12)
    half_epw = pdf.epw / 2
    spacing  = " " * 5  # 5 Leer­zeichen
    ...
    for _, row in eink_df.iterrows():
        ...
        # 1) Linke Spalte: Zutat + Menge + Einheit
        pdf.cell(pdf.get_string_width(left_text), 8, left_text, new_x=XPos.RIGHT, new_y=YPos.TOP)

        # 2) Berechne Einzug für die Kategorie:
        #    a) Mindest-Einzug: Breite von left_text + 5 Leerzeichen
        #    b) Seitenmitte: margin_left + epw/2
        margin_left = pdf.l_margin
        min_indent  = margin_left + pdf.get_string_width(left_text + spacing)
        center_x    = margin_left + half_epw
        indent_x    = max(min_indent, center_x)

        # 3) Kategorie ab dieser X-Position ausgeben
        pdf.set_x(indent_x)
        pdf.cell(0, 8, row["Kategorie"], new_x=XPos.LMARGIN, new_y=YPos.NEXT)


    # Abstand vor Kochliste
    pdf.ln(6)

    # 3) Koch­liste mit fetten Titeln und eingerückten Zutaten
    pdf.set_font("DejaVu", "B", 14)
    pdf.cell(0, 10, "Kochliste", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

    # import re steht bereits ganz oben, sonst hier hinzufügen
    # Filtere nur Zeilen, die mit "- *…*:" beginnen
    ...
    for l in lines:
        ...
        # 1) Titel fett mit Bullet
        pdf.set_x(pdf.l_margin)
        pdf.set_font("DejaVu", "B", 12)
        pdf.cell(0, 8, f"• {title}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # 2) Zutaten darunter eingerückt
        pdf.set_x(pdf.l_margin + 6)
        pdf.set_font("DejaVu", "", 12)
        pdf.multi_cell(pdf.epw - 6, 8, ingredients)

        pdf.ln(2)  # etwas Abstand zum nächsten Gericht


    # 4) Speichern & Senden
    filename = f"liste_{query.from_user.id}.pdf"
    pdf.output(filename)

    await query.edit_message_text("📄 Hier ist dein PDF mit Einkaufsliste und Kochliste:")
    with open(filename, "rb") as f:
        await query.message.reply_document(
            document=f,
            filename="Einkauf_und_Kochliste.pdf"
        )

    # Optional: Datei wieder löschen, wenn dein Server nicht volllaufen soll
    import os
    os.remove(filename)
...
async def fav_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    favs = favorites.get(user_id, [])
    context.user_data["fav_msgs"] = []       # IDs zum Aufräumen
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Ja", callback_data="fav_edit_yes"),
...
async def fav_overview_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === Nutzer will NICHT bearbeiten – alle Favoriten-Nachrichten löschen & Haupt-Buttons zeigen ===
    if q.data == "fav_edit_no":
        await mark_yes_no(q, False, "fav_edit_yes", "fav_edit_no")

        # alle gespeicherten Favoriten-Messages löschen
        msg = q.message
        for mid in context.user_data.get("fav_msgs", []):
            try:
                await q.bot.delete_message(chat_id=msg.chat_id, message_id=mid)
            except:
                pass

        # Callback-Message (Bearbeiten?-Buttons) löschen
        try:
            await msg.delete()
        except:
            pass

        # Hauptmenü wieder einblenden
        await send_main_buttons(msg)
        return ConversationHandler.END

    # === Nutzer will bearbeiten – Auswahl-Modus ============================
    if q.data == "fav_edit_yes":
        ...
        # Nummerierung jetzt korrekt ab 1
        text = "Welche Favoriten löschen?\n" + "\n".join(
            f"{i}. {d}" for i, d in enumerate(favs, start=1)
        )
        await q.message.reply_text(text)

        sel_msg = await q.message.reply_text(
            "Wähle Nummern (Mehrfachauswahl) und klicke »Fertig«:",
            reply_markup=build_fav_numbers_keyboard(len(favs), set())
        )
        context.user_data["fav_sel_msg_id"] = sel_msg.message_id
...
async def fav_number_toggle_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    num = int(q.data.split("_")[2])          # fav_del_3 → 3
    sel = context.user_data.setdefault("fav_del_sel", set())
    sel.symmetric_difference_update({num})   # toggeln
...
async def fav_delete_done_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Auswahl-Keyboard entfernen
    try:
        await q.bot.delete_message(chat_id=q.message.chat_id,
                                   message_id=context.user_data["fav_sel_msg_id"])
    except:
        pass

    # Falls jetzt leer → wie ganz am Anfang behandeln
    if not favs:
        await q.message.reply_text("Keine Favoriten vorhanden. Füge diese später hinzu!")
        await send_main_buttons(q.message)
        return ConversationHandler.END

    # Sonst zurück zu Übersicht (*)
    txt = "⭐ Deine Favoriten:\n" + "\n".join(f"- {d}" for d in favs)
    m1 = await q.message.reply_text(txt)
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Ja", callback_data="fav_edit_yes"),
                                InlineKeyboardButton("Nein", callback_data="fav_edit_no")]])
    m2 = await q.message.reply_text("Bearbeiten?", reply_markup=kb)
...
def main():
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            FAV_OVERVIEW:      [CallbackQueryHandler(fav_overview_cb,    pattern="^fav_edit_")],
            FAV_DELETE_SELECT: [
