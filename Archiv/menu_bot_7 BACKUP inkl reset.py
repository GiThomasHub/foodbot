...
def sample_by_weight(df: pd.DataFrame, weight: int, k: int) -> pd.DataFrame:
    """
    Gibt k Zeilen zurück, gemäss Schweregrad-Verteilung:
    weight 1 = nur 'leicht', …, weight 7 = nur 'schwer'.
    """
    # Aufteilen:
    df_light  = df[df["Art"] == "leicht"]
    df_medium = df[df["Art"] == "mittel"]
    df_heavy  = df[df["Art"] == "schwer"]

    # Verteilungsregeln gemäss Briefing
    mapping = {
        1: (1, 0, 0),
        2: (2, 1, 0),   # 2/3 leicht, 1/3 mittel
        3: (2, 3, 1),   # 2/6 leicht, 3/6 mittel, 1/6 schwer
        4: (1, 1, 1),   # ausgeglichen
        5: (1, 3, 2),
        6: (0, 1, 2),
        7: (0, 0, 1),
    }
    l, m, h = mapping[weight]
    total_parts = l + m + h
    counts = {
        "light":  int(k * l / total_parts),
        "medium": int(k * m / total_parts),
        "heavy":  k - int(k * l / total_parts) - int(k * m / total_parts),
    }

    parts = [
        df_light.sample(n=min(len(df_light),   counts["light"]),  replace=False),
        df_medium.sample(n=min(len(df_medium), counts["medium"]), replace=False),
        df_heavy.sample(n=min(len(df_heavy),   counts["heavy"]),  replace=False),
    ]
...
async def menu_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for stufe, menge in zip([1, 2, 3], [a1, a2, a3]):
            ...
            if not verf:
                continue                                              # nichts verfügbar
            picks = []
            for _ in range(min(len(verf), menge)):
                weights = [3 if d in favs else 1 for d in verf]
                choice  = random.choices(verf, weights=weights, k=1)[0]
                picks.append(choice);  verf.remove(choice)
            ausgewaehlt += picks
            aufwand_liste += [stufe] * len(picks)
            bereits.update(picks)

        if not ausgewaehlt:
            await update.message.reply_text("😕 Leider keine passenden Gerichte gefunden.")
            return MENU_INPUT
...
def profile_overview_text(p: dict) -> str:
    ...
    styles_str = ", ".join(p["styles"]) if p["styles"] else "–"
    return (
        "🗂 **Dein Profil**\n"
...
async def restart_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "Super, bis bald! Über den Button Menü kannst du jederzeit neu starten."
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("🍲 Menü",      callback_data="start_menu"),
        InlineKeyboardButton("🔖 Favoriten", callback_data="start_favs"),
        InlineKeyboardButton("🛠️ Übersicht",     callback_data="start_setup"),
...
async def reset_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔄 Alles wurde zurückgesetzt. Du kannst neu starten mit /menu.")
    return ConversationHandler.END
