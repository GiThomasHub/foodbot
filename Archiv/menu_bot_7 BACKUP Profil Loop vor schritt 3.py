...
async def profile_choice_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "prof_exist":
        # TODO: echte Profilprüfung folgt in Schritt 3
        await q.message.reply_text(
    ...
    # --- 3) Neues Profil --------------------------------------------------
    if choice == "prof_new":
        await q.message.reply_text("🛠️ Profil-Erstellung starten … (folgt im nächsten Schritt)")
        return ConversationHandler.END  # Platzhalter

    # --- 4) Mein Profil ---------------------------------------------------
    if choice == "prof_show":
        await q.message.reply_text("ℹ️ Profil-Übersicht … (folgt im nächsten Schritt)")
        return ConversationHandler.END  # Platzhalter
