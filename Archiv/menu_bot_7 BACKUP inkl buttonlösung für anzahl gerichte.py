...
async def ask_menu_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Fragt den Nutzer, wie viele Menüs er möchte (1–6).
    """
    buttons = [
        InlineKeyboardButton(str(i), callback_data=f"menu_count_{i}") for i in range(1, 7)
    ]
...
async def menu_count_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(
        f"{count} Menüs ausgewählt ✅\nJetzt den Aufwand bestimmen:",
...
def main():
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            ...
            MENU_COUNT: [CallbackQueryHandler(menu_count_cb, pattern=r"^menu_count_\d+$")],
