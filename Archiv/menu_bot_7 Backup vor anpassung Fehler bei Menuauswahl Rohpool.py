...
async def fertig_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for g in ausgew:
        ...
        for b in beilagen_namen:
            part_b = zut[(zut["Typ"] == "Beilage") & (zut["Gericht"] == b)]
            parts_list.append(part_b)

        # 4) Zusammenführen
        part = pd.concat(parts_list, ignore_index=True)

        # 5) Formatieren und zusammenfassen
        ze_parts = []
        for _, row in part.iterrows():
            if float(row["Menge"]) == 99:
                ze_parts.append(f"{row['Zutat']} wenig")
            else:
                amt = format_amount(row["Menge"])
                ze_parts.append(f"{row['Zutat']} {amt} {row['Einheit']}")
        ze = " | ".join(ze_parts)

        # 6) Titel mit Beilagen-Bezeichnung, dann Zutatenzeile anhängen
