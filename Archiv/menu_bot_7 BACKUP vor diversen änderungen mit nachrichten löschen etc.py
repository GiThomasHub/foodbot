...
async def profile_choice_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "prof_exist":
        if uid in profiles:
            ...
            # Flow-Nachrichten ab hier tracken
...
async def menu_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg2 = await update.message.reply_text("Passen diese Gerichte?", reply_markup=confirm_kb)
...
async def menu_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "confirm_no":
        ...
        # Swap-Prompt senden und **tracken** (wird später inkl. "Deine Gerichte" gelöscht)
...
async def quickone_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Startet QuickOne: sofort ein Gericht+Beilagen vorschlagen, ohne Wiederholungen."""
    uid = str(update.effective_user.id)

    # 1) Flow-Nachrichten-Tracking initialisieren
    context.user_data["flow_msgs"] = []

    # 2) Pool nur beim ersten Aufruf zurücksetzen
    data = update.callback_query.data if update.callback_query else None
    if (not update.callback_query) or data == "start_quickone":
        context.user_data.pop("quickone_remaining", None)

    # 3) Rest-Pool laden/initialisieren
    remaining = context.user_data.get("quickone_remaining")
    if not remaining:
        remaining = df_gerichte["Gericht"].tolist()

    # 4) Zufälliges Gericht ohne Wiederholung wählen
    dish = random.choice(remaining)
    remaining.remove(dish)
    context.user_data["quickone_remaining"] = remaining

    # 5) Beilagen ermitteln
    raw_codes = df_gerichte.loc[
        df_gerichte["Gericht"] == dish, "Beilage"
    ].iloc[0]
    codes      = parse_codes(raw_codes)
    side_nums  = choose_sides(codes)
    sides      = df_beilagen[
        df_beilagen["Nummer"].isin(side_nums)
    ]["Beilage"].tolist()
    ...
    # 7) Nachricht A: Gericht anzeigen
    chat_id = update.effective_chat.id
    text1 = f"🥣 *Dein Gericht:*\n\n{format_dish_with_sides(dish, sides)}"
    msg1 = await context.bot.send_message(
        chat_id=chat_id, text=text1, parse_mode="Markdown"
    )
    context.user_data["flow_msgs"].append(msg1.message_id)

    # 8) Nachricht B: Frage & Buttons separat
    text2 = "Passt das?"
    buttons = [
    ...
    msg2 = await context.bot.send_message(
        chat_id=chat_id, text=text2, reply_markup=markup
    )
    context.user_data["flow_msgs"].append(msg2.message_id)
...
async def quickone_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "quickone_neu":
        ...
        # 2) Neuen QuickOne-Durchgang starten
    ...
    if data == "quickone_beilagen_neu":
        ...
        # 3) Nachricht A: Gericht erneut anzeigen
        text1 = f"🥣 *Dein Gericht:*\n\n {format_dish_with_sides(dish, sides)}"
        msg1 = await context.bot.send_message(
            chat_id=chat_id, text=text1, parse_mode="Markdown"
        )
        ...
        # 4) Nachricht B: Frage & Buttons separat
        text2 = "Passt das?"
        buttons = [
            InlineKeyboardButton("Passt",         callback_data="quickone_passt"),
            InlineKeyboardButton("Neu!",     callback_data="quickone_neu"),
...
async def beilage_select_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "beilage_done":
        ...
        # 1) Kopfzeile
        text = "Deine Gerichte inkl. Beilagen:\n\n"

        # 2) Gerichte + formatierte Beilagen zusammenstellen
        ...
        # 4) Dann wie gewohnt nach Personen fragen
        return await ask_for_persons(update, context)
...
async def tausche_select_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "swap_done":
        ...
        msg1 = await q.message.reply_text(f"🔄 Neue Auswahl:\n{menutext}")
        context.user_data["flow_msgs"].append(msg1.message_id)
        ...
        msg2 = await q.message.reply_text("Passen diese Gerichte?", reply_markup=confirm_kb)
        context.user_data["flow_msgs"].append(msg2.message_id)
...
async def tausche_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "swap_again":
        ...
        # 1) Nur die letzte Frage löschen, nicht die "Neue Auswahl"
        mids = context.user_data.get("flow_msgs", [])
        if mids:
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=mids[-1])
            except:
                pass
            # Behalte nur die erste Nachricht in flow_msgs
            context.user_data["flow_msgs"] = [mids[0]]

        # 2) Prompt zum erneuten Tauschen senden und tracken
        kb = build_swap_keyboard(sessions[uid]["menues"], set())
        msg = await q.message.reply_text(
            "Welche Gerichte möchtest Du tauschen?",
            reply_markup=kb
    ...
    if q.data == "swap_ok":
        ...
        # 1) Nur die Frage-Meldung löschen, nicht die "Neue Auswahl"
        mids = context.user_data.get("flow_msgs", [])
        if len(mids) >= 2:
            question_mid = mids[-1]  # das letzte element ist "Passen diese Gerichte?"
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=question_mid)
            except:
                pass
            # behalten, was in mids[0] ist ("Neue Auswahl"), und neu setzen
            context.user_data["flow_msgs"] = [mids[0]]
        else:
            # Fallback: falls etwas schief lief, alle alte löschen
            for mid in mids:
                try:
                    await context.bot.delete_message(chat_id=chat_id, message_id=mid)
                except:
        ...
        # 2) Nachricht A: Bestätigung, dass es weitergeht
        msg1 = await q.message.reply_text("Gut, dann kümmern wir uns um die Beilagen.")
        context.user_data["flow_msgs"].append(msg1.message_id)

        # 3) Nachricht B: Frage nach Beilagen hinzufügen
        kb = InlineKeyboardMarkup([[  
            InlineKeyboardButton("Ja",   callback_data="ask_yes"),
            InlineKeyboardButton("Nein", callback_data="ask_no"),
        ]])
        msg2 = await q.message.reply_text(
            "Möchtest du Beilagen hinzufügen?",
            reply_markup=kb
        )
        context.user_data["flow_msgs"].append(msg2.message_id)
...
async def fav_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    m2 = await msg.reply_text("Favoriten entfernen?", reply_markup=kb)
