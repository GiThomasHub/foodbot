...
async def ask_menu_count(update: Update, context: ContextTypes.DEFAULT_TYPE, page: str = "low"):
    ...
    rows = [count_buttons[i:i + 3] for i in range(0, len(count_buttons), 3)]
    rows.append([nav_button])
    kb = InlineKeyboardMarkup(rows)

    text = "Wie viele Menüs möchtest du?"

    # Wenn CallbackQuery vorliegt, versuche zu editieren
    if update.callback_query and update.callback_query.message:
        try:
            await update.callback_query.message.edit_text(text, reply_markup=kb)
        except telegram.error.BadRequest as e:
            print("⚠️ Nachricht konnte nicht editiert werden, sende neu:", e)
    ...
    # Falls normaler Nachrichteneinstieg (z. B. erstes Mal)
    elif update.message:
