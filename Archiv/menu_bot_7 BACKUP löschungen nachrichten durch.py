...
async def ask_for_persons(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.callback_query:
        ...
        msg = await update.callback_query.message.reply_text(
            "Für wieviel Personen darf ucg die Einkaufs- und Kochliste erstellen?",
            reply_markup=markup
        )
    else:
...
async def menu_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 1) User klickt „Ja“ → Beilagen-Flow
    if query.data == "confirm_yes":
        # a) Haken setzen
        await mark_yes_no(query, True, "confirm_yes", "confirm_no")

        # b) Nur die Bestätigungs-Nachricht löschen,
        #    die Menü-Übersicht bleibt in flow_msgs erhalten
        flow = context.user_data.get("flow_msgs", [])
        if flow:
            last_msg_id = flow.pop()
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=last_msg_id)
            except:
                pass

        # c) Frage nach Beilagen senden
        kb = InlineKeyboardMarkup([[
    ...
    # 2) User klickt „Nein“ → Tausche-Loop starten
    if query.data == "confirm_no":
        # a) Haken setzen
        await mark_yes_no(query, False, "confirm_yes", "confirm_no")

        # b) Nur die Bestätigungs-Nachricht löschen,
        #    Menü-Übersicht bleibt sichtbar
        flow = context.user_data.get("flow_msgs", [])
        if flow:
            last_msg_id = flow.pop()
            try:
        ...
        # c) Swap-Candidates zurücksetzen
        uid = str(query.from_user.id)
        context.user_data["swap_candidates"] = set()

        # d) Tausche-Prompt senden
...
async def tausche_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "swap_again":
        ...
        # Nur die letzte Frage löschen (nicht die "Neue Auswahl")
        flow = context.user_data.get("flow_msgs", [])
        if flow:
            last_msg = flow.pop()
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=last_msg)
            except:
                pass
        # Flow-Msgs bestehen jetzt nur noch aus der "🔄 Neue Auswahl"-Nachricht

        # Erneut Tausche-Prompt senden
        kb = build_swap_keyboard(sessions[uid]["menues"], set())
        msg = await q.message.reply_text(
            "Welche Gerichte möchtest Du tauschen?",
            reply_markup=kb
    ...
    # 2) Alles passt → direkt in Beilagen-Flow
    if q.data == "swap_ok":
        await mark_yes_no(q, True, "swap_ok", "swap_again")

        # **Nur** die letzte Bestätigungs-Nachricht löschen,
        # die "🔄 Neue Auswahl" bleibt erhalten
        flow = context.user_data.get("flow_msgs", [])
        if flow:
            last_msg = flow.pop()
            try:
