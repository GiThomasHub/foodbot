    ...
    try:
        ...
        if "fav_selection" in context.user_data:
            ...
            if len(selected) == total:
                final_gerichte = selected
            elif len(selected) > total:
                final_gerichte = random.sample(selected, total)
            else:
                fehlend = total - len(selected)
                profile = profiles.get(user_id, default_profile())
                filters = context.user_data.get("filters", {})
                aufwand_liste = [1]*a1 + [2]*a2 + [3]*a3
                block = history.get(user_id, []) + selected

                extra = get_random_gerichte(
                    profile, filters, aufwand_liste, block=block,
                    limit=fehlend, mode="session"
                )
                final_gerichte = selected + extra

            sessions[user_id] = {
                "menues": final_gerichte,
                "aufwand": [1]*a1 + [2]*a2 + [3]*a3,
            }
...
def build_fav_selection_keyboard(total: int, selected: set[int]) -> InlineKeyboardMarkup:
    ...
    rows = [btns[i:i+7] for i in range(0, len(btns), 7)]
