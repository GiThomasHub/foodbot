...
async def tausche_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === q.data == "swap_ok" ===
    # 2) in den Beilagen Loop von Menu wechseln
    await q.edit_message_text("Gut, dann kümmern wir uns um die Beilagen.")

    # Stelle erneut die Ja/Nein-Frage zum Hinzufügen von Beilagen
    keyboard = [
        [InlineKeyboardButton("Ja",   callback_data="ask_yes")],
        [InlineKeyboardButton("Nein", callback_data="ask_no")],
    ]
    ...
    # Wechsle in den ASK_BEILAGEN-State, damit ask_beilagen_cb greift
    return ASK_BEILAGEN
