...
async def fertig_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Einkaufsliste für Export speichern
...
async def export_to_bring(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Option a): erstellt Deeplink und sendet Bring!-Button (async & sicher)"""
    query = update.callback_query
    await query.answer()
    ...
    recipe_json = {
        ...
        "@type":    "Recipe",
        "name":     "Einkaufsliste",
        "author": {"@type": "Organization", "name": "FoodApp"},  # <- Autor ergänzt (Fehler 2 präventiv umgehen)
        "recipeIngredient": recipe_ingredients,
    }

    # 2) Lokal zwischenspeichern (Debug / Backup)
    with open("einkaufsliste.json", "w", encoding="utf-8") as f:
        json.dump(recipe_json, f, ensure_ascii=False, indent=2)

    logging.info("Bring-Import JSON: %s", json.dumps(recipe_json, ensure_ascii=False))

    # 3) Upload & Deeplink komplett asynchron
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            # Beispiel-Upload als Raw-Gist – ersetze durch deinen PUT-Endpoint
            gist_raw_url = "https://gist.githubusercontent.com/GiThomasHub/7cc845bada3026ed9a7b643394d39ebb/raw/einkaufsliste.json"
            # await client.put(gist_raw_url, json=recipe_json, headers={"Content-Type": "application/json"})

            json_url = gist_raw_url   # hier liegt die Roh-JSON

            # Bring-API aufrufen
            # Neuer Code …
            dl_resp = await client.get(
                "https://api.getbring.com/rest/bringrecipes/deeplink",
                params={"url": json_url, "source": "web"},
                follow_redirects=False,                       # Bring! antwortet bewusst mit 302
            )

            if dl_resp.status_code in (301, 302, 303, 307, 308):
                deeplink = dl_resp.headers.get("location")   # z. B. https://go.getbring.com/3GnaegMak4eJvEpg6
                if not deeplink:
                    raise httpx.HTTPStatusError("Location-Header fehlt", dl_resp.request, dl_resp)
            else:
                dl_resp.raise_for_status()                   # echte Fehler ≧ 400
                # Fallback für den Fall, dass die API eines Tages doch JSON liefert
        ...
        logging.error("HTTP-Fehler bei Bring-Export: %s", err)
        await query.edit_message_text("❌ Bring-Export fehlgeschlagen. Bitte später erneut versuchen.")
        return ConversationHandler.END

    # 4) Button senden & Conversation beenden
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("In Bring! importieren", url=deeplink)]])
    await query.edit_message_text(
        "🛒 Tipp: Klick zum Import in Bring!",
        reply_markup=kb
    )
    return ConversationHandler.END
...
async def export_to_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Option b): Platzhalter für PDF-Export"""
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("📄 PDF-Export ist noch nicht implementiert.")
...
#>>>>>>>>>>>>CANCEL / RESET / RESTART | ACHTUNG: 2 MAL RESET DEFINIERT!!!!
