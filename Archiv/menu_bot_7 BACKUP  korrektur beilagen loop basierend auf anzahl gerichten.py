...
async def menu_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "confirm_yes":
        ...
        msg = await query.message.reply_text("Möchtest du Beilagen hinzufügen?", reply_markup=kb)
        context.user_data["flow_msgs"].append(msg.message_id)
        return ASK_BEILAGEN
    ...
    if query.data == "confirm_no":
        ...
        msg = await query.message.reply_text("Welche Gerichte möchtest Du tauschen?", reply_markup=kb)
        context.user_data["flow_msgs"].append(msg.message_id)
...
async def tausche_select_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "swap_done":
        ...
        msg1 = await q.message.reply_text(f"🥣 Deine Gerichte:\n{menutext}")
        context.user_data["flow_msgs"].append(msg1.message_id)
...
async def tausche_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if q.data == "swap_again":
        ...
        if flow:
            ...
            try:
                ...
                pass
        # erneuter Swap-Prompt
        kb = build_swap_keyboard(sessions[uid]["menues"], set())
        msg = await q.message.reply_text("Welche Gerichte möchtest Du tauschen?", reply_markup=kb)
        context.user_data["flow_msgs"].append(msg.message_id)
        return TAUSCHE_SELECT
    ...
    if q.data == "swap_ok":
        ...
        msg = await q.message.reply_text("Möchtest du Beilagen hinzufügen?", reply_markup=kb)
        context.user_data["flow_msgs"].append(msg.message_id)
        return ASK_BEILAGEN
