...
async def ask_beilagen_for_menu(update_or_query, context: ContextTypes.DEFAULT_TYPE):
    ...
    """
    # — Ziel für Nachrichten: entweder .message (bei Commands) oder .callback_query.message
    if getattr(update_or_query, "callback_query", None):
        target = update_or_query.callback_query.message
    else:
        target = update_or_query.message

    # 1) Prüfe, ob noch Slots offen sind
    idx_list = context.user_data.get("to_process", [])
    menu_idx = context.user_data.get("menu_idx", 0)
    if menu_idx >= len(idx_list):
        # alle Beilagen-Slots abgearbeitet → finale Frage „Liste anzeigen?“
    ...
    # 2) Welches Menü ist dran?
    idx = idx_list[menu_idx]
    menus = context.user_data["menu_list"]
    gericht = menus[idx]

    # 3) Roh-Beilage prüfen und evtl. überspringen
    raw = df_gerichte.loc[df_gerichte["Gericht"] == gericht, "Beilage"].iloc[0]
    codes = parse_codes(raw)
    if not codes or codes == [0]:
        # dieses Gericht hat keine Beilagen → zum nächsten springen
        context.user_data["menu_idx"] = menu_idx + 1
        return await ask_beilagen_for_menu(update_or_query, context)
...
async def tausche_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === q.data == "swap_ok" ===
    # a) Update-Text
    await q.edit_message_text("Gut, dann kümmern wir uns um die Beilagen.")

    # b) Aktuelle Menüs als Übersicht posten
    menus = sessions[str(q.from_user.id)]["menues"]
    menutext = "📋 Aktuelle Menüs:\n" + "\n".join(
        f"{i+1}. {g}" for i, g in enumerate(menus, 1)
    )
    await q.message.reply_text(menutext)

    # c) Kandidaten-Indices ermitteln
    swapped = context.user_data.get("swapped_indices", [])
    if swapped:
        candidates = [i-1 for i in swapped]           # 0-basiert
    else:
        candidates = list(range(len(menus)))           # alle Slots

    # d) Nur die mit echtem Beilagen-Code behalten
    to_process = []
    for idx in candidates:
        raw = df_gerichte.loc[
            df_gerichte["Gericht"] == menus[idx], "Beilage"
        ].iloc[0]
        codes = parse_codes(str(raw))
        if codes and codes != [0]:
            to_process.append(idx)

    # e) Wenn nichts übrig bleibt, direkt zur finalen Frage springen
    if not to_process:
        return await ask_showlist_cb(update, context)

    # f) Ansonsten den Beilagen-Flow für die echten Slots starten
    context.user_data["menu_list"]  = menus
    context.user_data["to_process"]  = to_process
    context.user_data["menu_idx"]    = 0
    return await ask_beilagen_for_menu(update, context)
