...
async def export_to_bring(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Option a): generiert Deeplink und sendet Bring!-Button"""
    query = update.callback_query
    await query.answer()
    ...
    # JSON-LD bauen
    recipe_ingredients = [
        f"{format_amount(row['Menge'])} {row['Einheit']} {row['Zutat']}"
        for _, row in eink.iterrows()
    ]
    recipe_json = {
    ...
    # → NEU: schreibe die JSON lokal in dein Projektverzeichnis
    ...
    # JSON auf eigenen Server hochladen (Beispiel)
    #upload_resp = requests.put(
    #    "https://mein-server.com/einkaufsliste.json",
    #    json=recipe_json,
    #    headers={"Content-Type": "application/json"}
    #)
    #upload_resp.raise_for_status()
    #json_url = upload_resp.url

    json_url = "https://a5ae4c0afed4.ngrok-free.app/einkaufsliste.json"

    # Deeplink von Bring! anfordern
    # Deeplink von Bring! anfordern – nur an den API-Endpoint!
    try:
        api_resp = requests.get(
            "https://api.getbring.com/rest/bringrecipes/deeplink",
            params={"url": json_url, "source": "app"},
            timeout=10
        )
        api_resp.raise_for_status()
    except HTTPError as e:
        text = api_resp.text if 'api_resp' in locals() else str(e)
        await query.edit_message_text(
            f"❗️ Bring!-API Fehler {e.response.status_code}:\n{text}"
        )
        return EXPORT_OPTIONS

    # JSON parsen und Deeplink extrahieren
    data = api_resp.json()
    deeplink = data.get("deeplink")
    if not deeplink:
        await query.edit_message_text(
            "❗️ Kein Deeplink in der Antwort gefunden:\n" + api_resp.text
        )
        return EXPORT_OPTIONS

    # *** NUR SENDEN – NICHT NOCH EINMAL REQUESTEN! ***
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("In Bring! importieren", url=deeplink)
    ]])
    await query.edit_message_text(
        "🛒 Klick hier, um deine Einkaufsliste in Bring! zu importieren:",
        reply_markup=kb
    )
    return EXPORT_OPTIONS
    ...
    #logging.info("Bring API Response: %d %s", dl_resp.status_code, dl_resp.text)
