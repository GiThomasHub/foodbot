...
async def fertig_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for g in ausgew:
        ...
        link = ""
        if "Link" in df_gerichte.columns:
            link = df_gerichte.loc[df_gerichte["Gericht"] == g, "Link"].iloc[0].strip()

        if link:
            # evtl. Sonderzeichen escapen, damit Markdown sauber bleibt
            safe = escape_markdown(title, version=1)
            md_title = f"*[{safe}]({link})*"           # ↩ Klickbarer Anchor-Text
        else:
            md_title = f"*{title}*"                  # wie bisher fett

        koch_text += f"{md_title}: {ze}\n"
    ...
    await context.bot.send_message(
        ...
        text=koch_text,
        parse_mode="Markdown",
        disable_web_page_preview=True,   # optional: keine Link-Previews
    )
    await context.bot.send_message(
        chat_id=chat_id,
        text=eink_text                   # kein parse_mode → kein Markdown-Stress
    )
...
async def process_pdf_export_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice in ["koch", "beides"]:
        ...
        lines = [l for l in koch_text.splitlines() if l.strip() and not l.startswith("🍽")]
        for l in lines:
            m = re.match(r"\*(?:\[(.+?)\]\([^)]+\)|(.+?))\*: (.+)", l)

            if not m:
                continue
            title = m.group(1) or m.group(2)      # Stern- oder Link-Variante
            ingredients = m.group(3)
