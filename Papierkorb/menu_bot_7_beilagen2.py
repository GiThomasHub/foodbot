...
async def ask_beilagen_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "ask_yes":
        ...
        # Buttons für alle Menüs, die Beilagen-Konfiguration ≠ 0 hat
        buttons = []
...
async def select_menus_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_done":
        if not sel:
            await query.message.reply_text("⚠️ Du hast keine Menüs ausgewählt. Vorgang abgebrochen.")
    ...
    # Toggle-Logik
    idx = int(data.split("_")[1]) - 1  # zurück auf 0-basiert
    if idx in sel:
        sel.remove(idx)
    else:
        sel.add(idx)

    # Buttons mit Häkchen neu rendern
    menus = context.user_data["menu_list"]
    buttons = []
    for i, gericht in enumerate(menus, start=1):
        if parse_codes( # falls Beilage==0, überspringen
