...
md = lambda t: escape_markdown(str(t), version=2)

async def send_md(bot, chat_id, text, **kw):
    """Sicher senden: Text wird escaped, parse_mode ist immer V2."""
    await bot.send_message(chat_id, md(text), parse_mode=ParseMode.MARKDOWN_V2, **kw)
...
def lade_gerichtebasis():
    ...
    df    = pd.DataFrame(
        data,
        columns=["Gericht", "Aufwand", "Typ", "Ernährungsstil", "Küche", "Beilagen"],
    )
...
async def menu_count_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_count_page_high":
        ...
        await query.message.edit_text(
            ...
            reply_markup=build_aufwand_keyboard(context.user_data["aufwand_verteilung"], count),parse_mode=ParseMode.MARKDOWN_V2
        )
        return MENU_AUFWAND
...
async def quickone_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 7) Gericht anzeigen
    text1 = f"🥣 *Dein Gericht:*\n{md(format_dish_with_sides(dish, sides))}"
    msg1 = await context.bot.send_message(chat_id, text=pad_message(text1), parse_mode=ParseMode.MARKDOWN_V2)
    context.user_data["flow_msgs"].append(msg1.message_id)
...
async def quickone_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "quickone_beilagen_neu":
        ...
        text1 = f"🥣 *Dein Gericht:*\n{md(format_dish_with_sides(dish, sides))}"
        msg1 = await context.bot.send_message(chat_id, text=text1, parse_mode=ParseMode.MARKDOWN_V2)
        context.user_data["flow_msgs"].append(msg1.message_id)
...
async def ask_beilagen_for_menu(update_or_query, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update_or_query.message.reply_text(
        pad_message(f"Wähle Beilagen für: *{md(gericht)}*"),
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode=ParseMode.MARKDOWN_V2
    )
    context.user_data["flow_msgs"].append(msg.message_id)
...
async def fertig_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for g in ausgew:
        ...
        title = md(format_dish_with_sides(g, beilagen_namen))
        koch_text += f"- *{title}*: {md(ze)}\n"
...
Anleitung (kurz Schritt-für-Schritt):"""
        ...
        msg = f"""📖 Rezept für *{md(dish)}* für *{personen}* Personen:
