...
def pad_message(text: str, min_width: int = 30) -> str:
    """
...
async def ask_for_persons(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Sendet die Inline-Buttons 1–6 + 'Mehr' und speichert die Message-ID in flow_msgs.
    ...
    if update.callback_query:
        ...
        msg = await update.callback_query.message.reply_text(
            "Für wieviel Personen soll ich die Einkaufs- und Kochliste erstellen?",
            reply_markup=markup
        )
    else:
    ...
    # Message-ID in flow_msgs tracken
...
def get_random_gerichte(profile, filters, aufwandsliste, block=None, limit=3, mode="session"):
    ...
    for stufe in aufwandsliste:
        ...
        kandidaten_liste = list(kandidaten["Gericht"])
        random.shuffle(kandidaten_liste)

        # Füge erstes nicht verwendetes Gericht dieser Stufe hinzu
        for gericht in kandidaten_liste:
            if gericht not in used:
                result.append(gericht)
                used.add(gericht)
                break  # max 1 Gericht pro Aufwandseintrag
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("🍲",      callback_data="start_menu"),
        InlineKeyboardButton("⚡",     callback_data="start_quickone"),
        InlineKeyboardButton("🔖", callback_data="start_favs"),
        InlineKeyboardButton("🛠️",     callback_data="start_setup"),
    ]])
    await update.message.reply_text(
...
async def profile_new_a_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.edit_text(
        pad_message("Stil auswählen (Mehrfachauswahl möglich):"),
...
async def ask_menu_count(update: Update, context: ContextTypes.DEFAULT_TYPE, page: str = "low"):
    """Fragt die Menüanzahl ab – mit zwei Seiten (1–6 und 7–12)."""
    if page == "low":
        buttons = [InlineKeyboardButton(str(i), callback_data=f"menu_count_{i}") for i in range(1, 7)]
        buttons.append(InlineKeyboardButton("Mehr ➡️", callback_data="menu_count_more"))
    else:
        buttons = [InlineKeyboardButton(str(i), callback_data=f"menu_count_{i}") for i in range(7, 13)]
        buttons.append(InlineKeyboardButton("⬅️ Weniger", callback_data="menu_count_less"))

    kb = InlineKeyboardMarkup([buttons[i:i+3] for i in range(0, len(buttons), 3)])

    # Ausgabe vorbereiten
    if update.callback_query:
        await update.callback_query.message.edit_text("Wie viele Menüs möchtest du?", reply_markup=kb)
    elif update.message:
        msg = await update.message.reply_text("Wie viele Menüs möchtest du?", reply_markup=kb)
        context.user_data["flow_msgs"] = [msg.message_id]
    else:
        chat_id = update.effective_chat.id
        msg = await context.bot.send_message(chat_id=chat_id, text="Wie viele Menüs möchtest du?", reply_markup=kb)
        context.user_data["flow_msgs"] = [msg.message_id]
...
async def menu_count_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_count_more":
        ...
        await query.message.edit_text(
            f"{count} Menüs ausgewählt. Jetzt den Aufwand bestimmen:",
...
async def start_menu_count_flow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Bestehende Flow-Messages löschen (nur zur Sicherheit)
    await cleanup_prof_loop(context, update.effective_chat.id)

    # Frage senden
    if isinstance(update, Update) and update.callback_query:
        await update.callback_query.answer()
        return await ask_menu_count(update.callback_query, context)
    else:
    ...
    try:
        ...
        if "fav_selection" in context.user_data:
            ...
            if show_debug_for(update):                                  #nur für ADMIN ersichtlich, as specified in def show_debug_for
                count = Counter(final_aufwand)
                debug_msg = (
                    f"📊 Aufwand-Verteilung: "
                    f"{count.get(1, 0)}x leicht, "
                    f"{count.get(2, 0)}x mittel, "
                    f"{count.get(3, 0)}x schwer"
                )
...
async def menu_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "confirm_yes":
        ...
        for idx, dish in enumerate(menus):
            raw = df_gerichte.loc[df_gerichte["Gericht"] == dish, "Beilage"].iloc[0]
        ...
        if not side_menus:
            ...
            for dish in menus:
                ...
                side_names = df_beilagen.loc[df_beilagen["Nummer"].isin(nums), "Beilage"].tolist()
        ...
        # 4b) genau 1 Beilagen-Menü: Einzel-Loop
        if len(side_menus) == 1:
            # Menu-Liste setzen für ask_beilagen_for_menu
        ...
        # 4c) >1 Beilagen-Menüs: Standard-Beilagen-Frage
        # Menu-Liste setzen
        context.user_data["menu_list"] = menus
        kb = InlineKeyboardMarkup([[
...
async def persons_selection_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Wir sammeln flow_msgs – keine person_msgs mehr
    if q.data == "persons_more":
        msg = await q.message.reply_text(pad_message("Gib die Anzahl bitte an:"))
        context.user_data.setdefault("flow_msgs", []).append(msg.message_id)
        return PERSONS_MANUAL

    # Direktzahl per Button
...
async def beilage_select_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "beilage_done":
        ...
        for dish in sessions[uid]["menues"]:
            nums = sessions[uid].get("beilagen", {}).get(dish, [])
            names = df_beilagen.loc[df_beilagen["Nummer"].isin(nums), "Beilage"].tolist()
    ...
    # Buttons neu aufbauen mit Markierungen für ausgewählte Beilagen
    buttons = []
    for code in context.user_data.get("allowed_beilage_codes", []):
        name = df_beilagen.loc[df_beilagen["Nummer"] == code, "Beilage"].iloc[0]
        mark = " ✅" if code in sel else ""
        buttons.append(
            InlineKeyboardButton(f"{name}{mark}", callback_data=f"beilage_{code}")
        )
    buttons.append(InlineKeyboardButton("Fertig", callback_data="beilage_done"))
    kb = [buttons[i:i+3] for i in range(0, len(buttons), 3)]
    await query.message.edit_reply_markup(InlineKeyboardMarkup(kb))
...
async def aufwand_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("aufwand_") and data.count("_") == 2:
        ...
        if op == "plus" and sum(verteilung.values()) < total:
            verteilung[key] += 1
        elif op == "minus" and verteilung[key] > 0:
            verteilung[key] -= 1
        ...
        # Fake-Eingabe-String erzeugen, um den bestehenden Flow zu nutzen
...
async def tausche_select_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "swap_done":
        # 1) Alle bisherigen Flow-Nachrichten löschen
        chat_id = q.message.chat.id
        for mid in context.user_data.get("flow_msgs", []):
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=mid)
        ...
        # 2) Swap-Logik ausführen
        menues = sessions[uid]["menues"]
        aufw   = sessions[uid]["aufwand"]
        # Sicherstellen, dass es überhaupt ein Beilagen-Dict gibt
        sessions[uid].setdefault("beilagen", {})
        # History initialisieren, falls nötig
        ...
        for idx in sorted(sel):             # idx ist 1-basiert
            ...
            if allowed:
                ...
                # Alte Beilagen-Liste des Slots löschen
...
async def restart_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 1) Abschieds-Nachricht
    await q.message.reply_text(pad_message("Super, bis bald!👋"))

    # 2) 5 Sekunden Pause
    await asyncio.sleep(3)

    # 3) Übersicht der Befehle senden
    overview = (
        "Du kannst jederzeit neu starten!\n\n"
        "🍲 Menü – Lass Dir leckere Gerichte vorschlagen\n\n"
        "⚡ QuickOne – Ein Gericht ohne Einschränkungen\n\n"
        "🔖 Favoriten – Deine Favoriten\n\n"
        "🛠️ Übersicht – Alle Funktionen"
    )
    await q.message.reply_text(overview)

    # 4) Aufforderung mit Buttons
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("🍲",      callback_data="restart_menu"),
        InlineKeyboardButton("⚡",  callback_data="restart_quickone"),
        InlineKeyboardButton("🔖",callback_data="restart_favs"),
        InlineKeyboardButton("🛠️", callback_data="restart_setup"),
    ]])
    await q.message.reply_text("Wähle eine Option:", reply_markup=kb)
