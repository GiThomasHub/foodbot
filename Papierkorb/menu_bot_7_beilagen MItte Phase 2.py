...
async def tausche_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1) Poste nochmal die aktuelle Menü-Liste als Übersicht
    ...
    # 2) Filtere nur Gerichte, die tatsächlich Beilagen benötigen
    swapped = context.user_data.get("swapped_indices", [])
    to_process = []
    for idx in swapped:  # idx ist bisher 1-basiert
        # Roh-Wert aus DataFrame holen (kann '0' oder 0 sein)
        raw_code = df_gerichte.loc[
            df_gerichte["Gericht"] == menus[idx-1], "Beilage"
        ].iloc[0]
        # Versuche, auf Integer zu casten
        try:
            code_int = int(raw_code)
        except (ValueError, TypeError):
            # Wenn das nicht geht, überspringe das Gericht
            continue
        # Nur weiter, wenn der Code wirklich > 0 ist
        if code_int != 0:
            to_process.append(idx - 1)

    context.user_data["to_process"] = to_process
    context.user_data["menu_list"]  = menus
    context.user_data["menu_idx"]   = 0

    # 3) Falls keine Gerichte mit Beilagen übrig, direkt zum Abschluss springen
    if not to_process:
        # call existing finale Frage „Liste anzeigen?“
        return await ask_final_list(q, context)

    # 4) Sonst: Ersten Beilagen-Dialog starten
